...
class SileroVAD:
    ...
    def __init__(self, threshold: float = VAD_THRESHOLD):
        ...
        self._loaded = False
        # Streaming inference (audio loop) and batch segmentation (worker
        # thread) both mutate _state/_context. ONNX inference releases the
        # GIL, so concurrent calls can corrupt state.
