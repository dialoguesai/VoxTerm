...
#   - the streaming zipformer (70M, chunk-16-left-128) int8 model, staged into the plugin's
#     Android assets so the APK transcribes fully offline (no first-run download)
...
cp "$CACHE/encoder-epoch-99-avg-1-chunk-16-left-128.int8.onnx" "$ASSETS/encoder.int8.onnx"
cp "$CACHE/decoder-epoch-99-avg-1-chunk-16-left-128.int8.onnx" "$ASSETS/decoder.int8.onnx"
cp "$CACHE/joiner-epoch-99-avg-1-chunk-16-left-128.int8.onnx"  "$ASSETS/joiner.int8.onnx"
